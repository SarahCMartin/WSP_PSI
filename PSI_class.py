class PSI:
    ...
    def PSI_master(self):
        ...
        if self.Emb_aslaid_model >= 10 and self.Emb_hydro_model >= 10 and self.Lat_brk_model >= 10 and self.Lat_res_model >= 10 and self.Ax_model >= 10: # soil always modelled as drained
            ...
            [su_consol_postlay, yield_stress_postlay, vert_eff_postlay] = PSI_soils.consolidation(PhaseNo, pressure_empty, self.t_aslaid, postlay_calc_depths, postlay_su_inc, self.gamma_sub, self.cv, self.SHANSEP_S, self.SHANSEP_m, self.z_aslaid, B_aslaid, [], [], 0)
            ...
            [int_su_consol_postlay, int_yield_stress_postlay, int_vert_eff_postlay] = PSI_soils.consolidation(PhaseNo, pressure_empty, self.t_aslaid, postlay_calc_depths, postlay_su_inc, self.gamma_sub, self.cv, self.int_SHANSEP_S, self.int_SHANSEP_m, self.z_aslaid, B_aslaid, [], [], 1)
        ...
        if self.Emb_aslaid_model >= 10 and self.Emb_hydro_model >= 10 and self.Lat_brk_model >= 10 and self.Lat_res_model >= 10 and self.Ax_model >= 10: # soil always modelled as drained
            ...
            [su_consol_hydro, yield_stress_hydro, vert_eff_hydro] = PSI_soils.consolidation(PhaseNo, pressure_hydro, self.t_hydro, hydro_calc_depths, hydro_su_inc, self.gamma_sub, self.cv, self.SHANSEP_S, self.SHANSEP_m, self.z_hydro, B_hydro, yield_stress_postlay, vert_eff_postlay, 0)
            [int_su_consol_hydro, int_yield_stress_hydro, int_vert_eff_hydro] = PSI_soils.consolidation(PhaseNo, pressure_hydro, self.t_hydro, hydro_calc_depths, int_hydro_su_inc, self.gamma_sub, self.cv, self.int_SHANSEP_S, self.int_SHANSEP_m, self.z_hydro, B_hydro, int_yield_stress_postlay, int_vert_eff_postlay, 1)
        ...
        if self.Emb_aslaid_model >= 10 and self.Emb_hydro_model >= 10 and self.Lat_brk_model >= 10 and self.Lat_res_model >= 10 and self.Ax_model >= 10: # soil always modelled as drained
            ...
            [su_consol_preop, yield_stress_preop, vert_eff_preop] = PSI_soils.consolidation(PhaseNo, pressure_preop, self.t_preop, hydro_calc_depths, su_consol_hydro, self.gamma_sub, self.cv, self.SHANSEP_S, self.SHANSEP_m, self.z_hydro, B_hydro, yield_stress_hydro, vert_eff_hydro, 0)
            [int_su_consol_preop, int_yield_stress_preop, int_vert_eff_preop] = PSI_soils.consolidation(PhaseNo, pressure_preop, self.t_preop, hydro_calc_depths, int_su_consol_hydro, self.gamma_sub, self.cv, self.int_SHANSEP_S, self.int_SHANSEP_m, self.z_hydro, B_hydro, int_yield_stress_hydro, int_vert_eff_hydro, 1)
        ...
        if self.Lat_res_model == 0: # in-situ yield stress ratio and existing vertical stresses needed for SHANSEP part of this model
            ...
            [_, int_yield_stress_res, int_vert_eff_res] = PSI_soils.consolidation(PhaseNo, pressure_empty, 0, res_calc_depths, res_su_inc, self.gamma_sub, self.cv, self.int_SHANSEP_S, self.int_SHANSEP_m, self.z_res, B_res, [], [], 0)
